...
class TodoController extends Controller
{
    ...
    public function createAction(Request $request){
        ...
        $form=$this->createFormBuilder($todo)
            ...
            ->add('due_date',DateTimeType::class, array('attr'=>array(
                'class'=>'formcontrol','style' => 'margin-bottom:15px')))
    ...
    public function editAction($id, Request $request){
        ...
        $form=$this->createFormBuilder($todo)
            ...
            ->add('due_date',DateTimeType::class, array('attr'=>array(
                'class'=>'formcontrol','style' => 'margin-bottom:15px')))
            ->add('save',SubmitType::class,array('label'=> 'Update todo','attr'=>array(
                'class'=>'btn btn-primary','style' => 'margin-bottom:15px')))
            ->getForm();
        $form->handleRequest($request);
        if ($form->isSubmitted()&& $form ->isValid()){
            //GET Data
            $name =$form['name']->getData();
            $categery =$form['category']->getData();
            $description =$form['description']->getData();
            $priority =$form['priority']->getData();
            $due_date =$form['due_date']->getData();
